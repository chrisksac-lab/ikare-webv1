...
function ChatScreen({navigation, route}) {
  ...
  const handleAddAttachment = (files) => {
   ...
   const pickDocument = async () => {
    ...
    const getActiveUser = async() => {
        ...
        try {
            const response = await axiosInstance.post(url, {id: activeUser?._id, hospitalName:JSON.parse(await getValue("hospital"))?.name})
  ...
  return (
    <View style={tw`h-full`}>
        <View style={tw`bg-[#0C3778] mt-7 p-4 flex flex-row justify-between items-center`}>
             ...
             {
                contact?.image ?
                <Image style={tw`rounded-[150px] mr-2`} source={{uri: axiosInstance.getUri() + contact.image}} width={50} height={50} />
                :
        ...
        <View style={tw`p-4 h-[80%]`}>
            {
                ...
                <FlatList 
                 ...
                 ListEmptyComponent={() => (
                    <View style={tw`p-4`}>
                      <Text style={tw`text-center text-[15px] font-semibold`}>Enter a message and click on send</Text>
                    </View>
                 )}
        ...
        <View style={tw`flex flex-row justify-between items-center absolute bottom-3 left-0 right-0`}>
                <View style={tw`p-4 rounded-5 bg-gray-700 w-[90%] flex flex-row justify-start gap-2 items-center`}>
                    <MaterialIcons 
